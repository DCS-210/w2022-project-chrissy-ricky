...
```{r vaccination-map, echo = FALSE}
vaccinations <- covid_data %>%
  filter(date == "2022-02-18") %>%
  filter(is.na(continent) == FALSE) %>%
  mutate(vaccination_per_person = total_vaccinations/population) %>%
  select(iso_code, location, date, vaccination_per_person) %>%
  na.omit()
...
vac_map <- vaccinations %>% 
  left_join(world_spdf2, by = c("location" = "NAME")) %>%
  st_as_sf()

pal2 <- colorNumeric(palette = "RdYlGn", domain = vac_map$vaccination_per_person)
labels <- sprintf("<strong>%s</strong><br/>%s ", vac_map$location, vac_map$vaccination_per_person) %>%
  lapply(htmltools::HTML)

v_map <- leaflet(data = vac_map) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=1) %>%
  addPolygons(fillColor = ~pal2(vac_map$vaccination_per_person),
                                 fillOpacity = 1, 
                                 color = "white",
                                 opacity = 0.7,
                                 weight = 1,
                                 label = labels) %>%
  addLegend("bottomleft", pal = pal2, values = ~vac_map$vaccination_per_person,
    title = "Vaccinations per Person",
    opacity = 1)
